...
static void render(layer_t *l) {
  ...
  uint32_t dst_w = b->render_w;
  uint32_t dst_h = b->render_h;

  // Allocate new size bitmap
  if(l->bitmap.data) rgba_free(&l->bitmap);
  l->bitmap = rgba_alloc(dst_w, dst_h, NULL);

  // Easy access
  uint32_t *src = b->original.data;
...
static void bitmap_delete(layer_t *l) {
  bitmap_t *b = (bitmap_t*)l;
  if(b->original.data == l->bitmap.data) l->bitmap.data = NULL;
  ...
  if(upng_get_error(upng) == UPNG_EOK) {
    ...
    if(upng_get_error(upng) == UPNG_EOK) {
      if(upng_get_format(upng) == UPNG_RGBA8) {
        ...
        // Convert to RGBA
        l->bitmap = b->original = rgba_alloc(upng_get_width(upng), upng_get_height(upng), upng_get_buffer(upng));
      } else {
